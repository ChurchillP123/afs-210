...
def dijsktra(graph, initial, end, Bool):
...
[1])
    ...
    # Reverse path
    path = path[::-1]
    return weight_to_current_node if Bool else path
        
print(f"Shortest path between a and h is {breadth_first_search(graph, 'A', 'H')}")
print(f"Cheapest path between a and h is {dijsktra(graph, 'A', 'H', False)}")
print(f"The cost of going between a and h is {dijsktra(graph, 'A', 'H', True)}")